...
if st.button('Distribution of Sales For each Company'):
    fig = px.histogram(df_selected_final, x="Global_Sales", labels=dict(Global_Sales="Total Game Sold in M"), title='Distribution of the Global Sales across platforms')
    st.plotly_chart(fig)
...
if st.button('All the Games and the Genre'):
    fig = px.histogram(df_selected_final, x="Genre",title="Genre Across Different Platform")
    st.plotly_chart(fig)

st.subheader('The top sold video game genre:')
num_vg=st.slider('Display the top N Genres:')
fig = px.histogram(df_selected_final.head(num_vg), x="Genre",title='Displaying the Top Games in Terms of Global Sales and Their genre/platform')
st.plotly_chart(fig)
...
df_N=df_N[:num_vg]
fig = px.histogram(df_selected_final.head(num_vg), x="Genre",title='Displaying the Top Games in Terms of Global Sales and Their Publisher')
st.plotly_chart(fig)
...
fig = px.bar(final[final["Year"]<2017] , x="Company", y="Cumulative_Sales",
...
fig = px.histogram(df.head(num_vg), x="Genre")
st.plotly_chart(fig) 
